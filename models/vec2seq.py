...
class Baseline(Abstract_Gen_Model):
    ...
    def forward(self, va_feat, questions):
        ...
        packed_embeddings = pack_padded_sequence(embeddings, lengths, batch_first=True) # add additional image feature
        feats, _ = self.rnn(packed_embeddings, hidden_feat)
        unpacked_feats, lengths_unpacked = pad_packed_sequence(feats, batch_first=True)
        #print(unpacked_feats.size())
        if __DEBUG__:
            print("[Generation Module] RNN feature.std(): "),
            print(feats.std())
        new_unpacked_feats = unpacked_feats.reshape(unpacked_feats.size(0), -1)

        pred = self.classifier(feats.data)
        #print(pred.size())
        #print(len(lengths))
        #print(embeddings.size())
        pred = PackedSequence(pred, batch_sizes=lengths_unpacked)
        #pred = pack_padded_sequence(pred, lengths_unpacked) 
        pred, unpacked_ls = pad_packed_sequence(pred, batch_first=False)
        #pred= pad_packed_sequence([pred, feats.batches, batch_first=True)
        #print(pred.size())
        #exit()
        #pred = pred.index_select(0, inv_ids)
